...
class Login : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnIniciarSesion.setOnClickListener {
            ...
            //Use el lifecycleScope en vez de GlobalScope Para asegurar que las corutinas se cancelen automáticamente cuando la actividad se termine.
            lifecycleScope.launch(Dispatchers.IO) {
                try {
                    val objConexion = Conexion().cadenaConexion()
                    val comprobarUsuario = objConexion?.prepareStatement("SELECT * FROM Usuario WHERE nombre_usuario = ? AND contra_usuario = ?")
